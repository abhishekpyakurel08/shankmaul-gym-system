...
import { User, Member, Subscription, Attendance, Income, Expense, Training } from '../models';
...
const seedComprehensive = async () => {
    try {
        ...
        if (!staff) {
            staff = await User.create({ email: staffEmail, password: 'staff123', role: 'staff' });
        }

        const trainerEmail = 'trainer_pro@shankhamul.com';
        let trainerUser = await User.findOne({ email: trainerEmail });
        if (!trainerUser) {
            trainerUser = await User.create({ email: trainerEmail, password: 'trainer123', role: 'trainer' });
        }
        ...
        for (const m of memberData) {
            ...
            if (!u) {
                u = await User.create({ email: m.email, password: 'password123', role: 'member' });
            }

            let member = await Member.findOne({ user: u._id });
            if (!member) {
        ...
        for (const t of trainingPrograms) {
            await Training.findOneAndUpdate({ name: t.name }, t, { upsert: true });
        }
        ...
        const expAmts: any = { rent: 45000, salaries: 60000, equipment: 15000, utilities: 8000, marketing: 5000, other: 3000 };
